...
const EducationInput = ( { educationInfo, setEducationInfo }: Props) => {
	...
	function handleSubmit(e: FormEvent): void {
		...
		if (degreeRef.current && schoolRef.current && cityRef.current && startRef.current && endRef.current && countryRef.current) {
		...
		<InputWidget openState={open}>
			...
			>
				...
				<form
					...
					className={`p-6 flex flex-col gap-6  ${open ? 'border-t-2' : 'invisible h-0 p-0'}`}
				>
					<InputWrapper
						HTMLfor="degree"
						labelText="Degree"
			...
			&&  <div className="border-t-2">
					{educationInfo.map(edu => {
						return (
							<div key={edu.id} className="flex justify-between p-6 text-sm">
								<div>
								...
								<div className="self-center">
									<button 
										...
										onClick={() => {
											...
											setEditID(edu.id)
											setOpen(true);
											degreeRef.current!.value = edu.degree;
											schoolRef.current!.value = edu.school;
											cityRef.current!.value = edu.city;
											countryRef.current!.value = edu.country;				
											startRef.current!.value = edu.startDate;
											endRef.current!.value = edu.endDate; 
										}}
									>
										<Image src={editButton} alt="" className="w-[24px]"/>
										Edit
									</button>
								</div>
							</div>
